					...
					<div class="user-panel">
						...
						<?php
								...
								<div class="dropdown-content">
								  <a href="../account/validar.php">Conta</a>
						...
						<div class="up-item">
							...
							<a style="cursor:pointer" href="cart.php">Lista de desejos</a>
			...
			<li><a href="/StormStore/products/extra/category.php">LANÇAMENTOS</a></li>
				...
				<li><a href="#">INFANTIL</a>
					<ul class="sub-menu">
						...
						<li><a href="/StormStore/products/infantil/tenis.php">Tênis</a></li>
					</ul>
				</li>
				<li>
...
<br>
	...
	<div class="page-top-info" >
		...
		<?php
				if (isset($_COOKIE["User"])) {
					$totalPrice = 0;
					echo "<h4>Carrinho de $username</h4><hr><br>";
					for ($i = 0; $i < sizeof($cartResult); $i++) {
						$productID = $cartResult[$i]["product_id"];
						$productQuery = Database::query("SELECT * FROM products WHERE id = ?", [$productID]);
						if (sizeof($productQuery) == 0) break;
						$productName = $productQuery[0]["name"];
						$productPrice = $productQuery[0]["price"];
						$productConteudo = $productQuery[0]["conteudo"];
						$productTam = $productQuery[0]["size"];
						$productMarca = $productQuery[0]["marca"];
						$totalPrice += floatval(str_replace("R$", "", $productPrice));
						echo "<div class='product-item'> 
						<div class='pi-pic'style='width: 20%'>
						<img src= '../img/logo.jpeg'>	
							<div class='pi-links'>
								<a href='#' class='add-card' product_id='$productID'><i class='flaticon-bag'></i><span>REMOVER</span></a>
							</div>
						</div>
						<div class='pi-text' charset='UTF-8'>
							<h6>$productPrice</h6>
							<p style='font-size:25px;'><strong>$productName</strong></p>
							<h6 style='charset='UTF-8',float:left; font-size: 15px;'> Produto da $productMarca , $productConteudo, tamanho $productTam</h6>
							
						</div><hr>
					</div>";
					}
					$totalPriceInString = number_format($totalPrice, 2, ",", "");
					echo "<center><h4>Preço total: R$$totalPriceInString</h4><br>
					<form method='POST' action='../checkout 1/index.php'>
					<button type='submit' class='enviarbtn'style='cursor: pointer;background-color:black;border-radius: 50px; color: white; padding: 12px 50px; font-size: 16px'>
							Finalizar Compra</button></center></form>";
				} else {
					echo "<center><h4>Você não está logado. Logue <a href='../account/login.php'>aqui</a> para poder acessar o seu carrinho!</h4></center>";
				}
			?>
		</div>
	</div>
